...
from .aiortc_source import RTCPeerConnection, RTCSessionDescription, RTCDataChannel, RTCConfiguration, RTCIceServer
...
class TunnelClient:
    def __init__(self, host: str, port: int, destination_port: int, signal_server, destination, topic_name: str, stun_server: str):
        self._host = host
        self._port = port
        ...
        self._stun_server_ip = stun_server

    async def run_async(self):
        logging.info('[INIT] Creating RTC Connection')
        #self._peer_connection = RTCPeerConnection()
        self._peer_connection = RTCPeerConnection(configuration=RTCConfiguration([RTCIceServer(str("stun:"+self._stun_server_ip))]))
        self._create_healthcheck_channel()
        await self._peer_connection.setLocalDescription(await self._peer_connection.createOffer(self._topic_name))

        logging.info('[INIT] Connecting with signaling server')
        client2 = await self._signal_server.connect_async()

        logging.info('[INIT] Sending local descriptor to signaling server')
        print("\n\ninvio:", self._peer_connection.localDescription, "\n\ndestination:", self._destination, "\n\n")
        #self._signal_server.send(self._peer_connection.localDescription, self._destination)
        await self._signal_server.send_data(client2, self._peer_connection.localDescription, self._destination)

        logging.info('\n\n[INIT] Awaiting answer from signaling server')
